...
* 너무 많은 독립변수를 갖는 모델에 패널티를 부과하여 과적합(과대적합)을 방지하는 방법

* 모델 성능을 저하하는 변수의 영향력을 축소시키거나 제거한다.

* MSE(mean sqaured error)를 최소화하는 기울기(w)와 절편(b)를 찾는 회귀 알고리즘의 error 값에 alpha에 비례한 값을 더하여 규제 강화
:::info MSE란?
* MSE: 실제 값과 예측값의 차이 제곱의 평균

> $MSE=1/N\sum_{i=0}^{N-1}(y_i-\bar y_i)^2=1/N\sum_{i=0}^{N-1}(y_i-(wx_i+b))^2$

이 때, 회귀 알고리즘은 이 error 값을 최소화하는 방향으로 학습하기 때문에 error값을 크게 할수록(alpha가 커질수록) 모델이 단순해짐을 뜻한다.

## 릿지 회귀

* $Error=MSE+aw^2$
* 독립변수의 척도에 큰 영향을 받으므로 표준화 선행 필요
* 어떠한 회귀계수도 0으로 만들지 않음
    * 기존의 데이터셋에 있는 모든 변수가 그대로 적용된다는 뜻

## 라쏘 회귀

* $Error=MSE+a\left\vert w \right\vert$
* 라쏘 회귀분석: 모델의 성능에 기여하지 못하는 독립변수의 회귀계수를 0으로 설정
    * 변수를 모델에서 제거함을 의미 -> 더욱 간단한 모델을 만들 수 있음

:::danger 릿지 회귀와 라쏘 회귀의 비교
* 라쏘 회귀는 독립변수를 제거할 수 있으므로 독립변수가 모델에 끼치는 영향력의 정도가 극단적일수록 우수한 성능을 가진다.
* 릿지 회귀는 변수의 영향력을 조절하기 때문에 영향력의 정도가 크지 않을수록 우수한 성능을 가진다.
:::
